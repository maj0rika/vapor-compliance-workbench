...
describe('PreviewPanel', () => {
  ...
  it('생성물 워크스페이스와 Canvas 기본 탭을 렌더링한다', () => {
    ...
    expect(screen.getByTitle('생성물 Canvas 미리보기')).toBeInTheDocument();
    expect(screen.getByTitle('생성물 Canvas 미리보기')).toHaveAttribute(
      'sandbox',
      'allow-scripts allow-same-origin',
    );
    expect(new URL((screen.getByTitle('생성물 Canvas 미리보기') as HTMLIFrameElement).src).origin)
      .not.toBe(window.location.origin);
    expect(screen.getByText('메타데이터: 통과')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Disabled 상태' })).toBeInTheDocument();
    expect(screen.getByLabelText('미리보기 런타임: 로딩')).toBeInTheDocument();
  });
  ...
  it('Canvas runtime ready/error message 를 parent UI 에 반영한다', async () => {
    render(<PreviewPanel draft={ARTIFACT} artifactSource={ARTIFACT_SOURCE} onClose={vi.fn()} />);
    const iframe = screen.getByTitle('생성물 Canvas 미리보기') as HTMLIFrameElement;
    ...
    await screen.findByLabelText('미리보기 런타임: 준비됨');
  ...
  it('artifact-meta 가 없으면 휴리스틱 props 사용을 명시한다', () => {
    ...
    expect(screen.getByText('추정 props')).toBeInTheDocument();
  ...
  it('metadata contract 실패를 Canvas unavailable 로 표시한다', () => {
    ...
    expect(screen.getByText('메타데이터: 실패')).toBeInTheDocument();
    expect(screen.getByText(/메타데이터 검증 실패/)).toBeInTheDocument();
    expect(screen.queryByTitle('생성물 Canvas 미리보기')).not.toBeInTheDocument();
  });
  ...
  it('artifactSource 가 없으면 Canvas 를 실제 preview 로 위장하지 않는다', () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('실제 미리보기 런타임');
  });
  ...
  it('Component 탭에서 생성 코드를 확인할 수 있다', () => {
    ...
    expect(screen.getByRole('tab', { name: '코드' })).toHaveAttribute(
      'aria-selected',
      'true',
    );
  ...
  it('runner 결과 전에는 정적 검증 badge 를 표시하지 않는다', () => {
    ...
    expect(screen.queryByText('Typecheck: PASS')).not.toBeInTheDocument();
    expect(screen.queryByRole('tab', { name: '검증' })).not.toBeInTheDocument();
  });
  ...
  it('deterministic sample provenance 와 validation 대기 상태를 표시한다', () => {
    ...
    expect(screen.getByLabelText('검증 샘플 출처')).toHaveTextContent(
      'API 호출 없음',
    );
  ...
  it('생성물이 비어 있으면 안내 문구를 보여준다', () => {
    ...
    expect(screen.getByText('접근성 + 토큰 확인')).toBeInTheDocument();
  });
  ...
  it('Canvas runtime timeout 은 failed 와 구별되는 4번째 상태다', async () => {
    ...
    expect(screen.getByLabelText('미리보기 런타임: 지연')).toBeInTheDocument();
    expect(
      screen.getByText('Canvas 런타임 응답 없음'),
    ).toBeInTheDocument();
    // Must NOT show as 'failed'
    expect(screen.queryByLabelText('미리보기 런타임: 실패')).not.toBeInTheDocument();
  ...
  it('탭 본문에 role="tabpanel" + aria-labelledby + id 가 연결된다', () => {
    ...
    const canvasTab = screen.getByRole('tab', { name: '미리보기' });
  ...
  describe('Repair attempts limit (U06)', () => {
    ...
    it('attempts 가 max 미만이면 "실패 수정" 버튼이 활성화된다', async () => {
    ...
    it('한도 prop 이 없으면 (기존 호출자 호환) 항상 enabled', async () => {
      ...
      // 헤더에 별도 카운터 텍스트를 노출하지 않는다.
      expect(screen.queryByText(/수정 \d+\/\d+/)).not.toBeInTheDocument();
    });
  });
});
